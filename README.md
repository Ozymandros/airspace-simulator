...
A browser-based 3D airspace simulation where fifteen autonomous aircraft fly inside a bounded volume over procedural terrain. Built as a single self-contained HTML file using [Three.js](https://threejs.org/) for rendering and [Yuka](https://mugen87.github.io/yuka/) for steering and flocking behavior.
...
- **Autonomous fleet** — 15 color-coded aircraft with realistic, smoothed turns and continuous motion
...
- Low-poly aircraft models (fuselage, wings, tail) with emissive highlighting when tracked
...
A `Smoother` on orientation produces banking-style turns instead of instant snaps. A minimum airspeed guard prevents aircraft from stalling in place.
